...
abstract contract StratX4 is Ownable, ReentrancyGuard, Pausable {
    ...
    function _farmSpecificAmount(uint256 _amount) internal virtual {
        require(isAutoComp, "!isAutoComp");
        require(_amount <= IERC20(wantAddress).balanceOf(address(this)), "Insufficient want amount");
        if(_amount <= 0){
            return;
        }
        wantLockedTotal = wantLockedTotal.add(_amount);
        IERC20(wantAddress).safeIncreaseAllowance(farmContractAddress, _amount);

        if (isCAKEStaking) {
            IPancakeswapFarm(farmContractAddress).enterStaking(_amount); // Just for CAKE staking, we dont use deposit()
        } else {
            IPancakeswapFarm(farmContractAddress).deposit(pid, _amount, BECORefAddress);
        }
    }
    ...
    function earn() public virtual whenNotPaused onlyOwner {
        ...
        if (isCAKEStaking || isSingleStaking) {
            ...
            _farmSpecificAmount(earnedAmt.div(2));
...
// File: contracts/StratX4_SAME_BECO.sol
...
contract StratX4_SAME_BECO is StratX4 {
    constructor(
